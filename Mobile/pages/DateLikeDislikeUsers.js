...
const JobLikeDislikeUsers = ({ route, navigation }) => {
    ...
    const getJobDetail = async () => {
        try {
            ...
            if (userTokenValue) {
                setLoading(true);
                const response = await axios.get(`https://nexusmain.onrender.com/api/date/${dateId}/likes`, {
                    headers: {
    ...
    const handleSwipeLeft = async (cardIndex) => {
        ...
        try {
            const userTokenValue = await AsyncStorage.getItem('usertoken');
            if (userTokenValue) {
                await axios.post(`https://nexusmain.onrender.com/api/jobs/${dateId}/move-user/${userId}`, {}, {
                    headers: {
                        Authorization: `Bearer ${userTokenValue}`,
                    },
                });
